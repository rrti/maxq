...
class State:
	NUM_ROWS = 5
	NUM_COLS = 7
	WALLS = [(0, 0, NUM_COLS, 'H'), (0, 0, NUM_ROWS, 'V'), (NUM_ROWS, 0, NUM_COLS, 'H'), (0, NUM_COLS, NUM_ROWS, 'V'), (1, 4, 3, 'V')]
	COORS = {'R': [1, 1], 'G': [1, 0], 'B': [3, 0], 'Y': [3, 1], 'Z': [4, 6], '+': [0, 4], '-': [4, 4]}
	SPAWNPOINTS = ['R', 'G', 'B', 'Y']


	def __init__(self, unit1Loc = 'G', unit2Loc = 'Y', transPos = [2, 0], enemyPos = [1, 3, 1]):
	...
	def randomize(self):
		"""
		self.unit1Loc = random.choice(self.SPAWNPOINTS)
		self.unit2Loc = random.choice(self.SPAWNPOINTS)
		self.transPos = [2, 0]
		self.enemyPos = [random.randint(0, State.NUM_ROWS - 1), 3, random.choice([-1, 1])]

		while ((self.unit1Loc == self.unit2Loc)):
